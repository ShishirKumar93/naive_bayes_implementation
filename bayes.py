...
class defaultintdict(dict):
    """
    Behaves exactly like defaultdict(int) except d['foo'] does NOT add 'foo' to dictionary d
    """
    def __init__(self):
        self._factory=int
        super().__init__()
...
def vocab(neg:Sequence[Sequence], pos:Sequence[Sequence]) -> dict:
    ...
    V = defaultintdict()
    V['__unknown__'] = 0
    flat_neg = [word for word_list in neg for word in word_list]
    flat_pos = [word for word_list in pos for word in word_list]
    combined_list = set().union(flat_neg, flat_pos)
...
def vectorize(V:dict, docwords:Sequence) -> np.ndarray:
    ...
    #word_cnt =  [(word,docwords.count(word)) for word in set(docwords)]    
...
def vectorize_docs(docs:Sequence, V:dict) -> np.ndarray:
    ...
    """
    D = [vectorize(V, word_list) for word_list in docs]
    return np.vstack(D)
